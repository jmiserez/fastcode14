...
void weights(uint32_t nimages, uint32_t npixels, uint32_t r, uint32_t c, double contrast_parm, double sat_parm, double wexp_parm, double **I, double **W);
...
//    uint32_t nvals = mem->nvals;
    ...
    PERF_FUNC_ENTER
    weights(nimages,npixels,r,c,contrast_parm,sat_parm,wexp_parm,I,W);
    PERF_FUNC_EXIT
...
FORCE_INLINE void _weights_inner(double *im, int at3, int at, int top, int left, int right, int bottom, double *Wn, double sat_parm, double contrast_parm, double wexp_parm){
    double r = im[at3];
    double g = im[at3+1];
    double b = im[at3+2];
    double r1 = im[top];
...
FORCE_INLINE void _weights_inner_row(double *im, int c, int i, int itop, int ibottom, double *Wn, double sat_parm, double contrast_parm, double wexp_parm){
    ...
    for(int j = 1; j < c-1; j++){
        ...
        _weights_inner(im,at3,at,top,left,right,bottom,Wn,sat_parm,contrast_parm,wexp_parm);
    }
    //left cols
    int j = 0;
    double at = (i*c+j);
