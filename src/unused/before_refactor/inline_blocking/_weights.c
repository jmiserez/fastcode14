...
FORCE_INLINE void _weights_inner(double *im, double **imC, int at, int real_at, int top, int left, int right, int bottom, double *Wn, double sat_parm, double contrast_parm, double wexp_parm){
    double r = im[at];
    double g = im[at+1];
    double b = im[at+2];
...
FORCE_INLINE void _weights_fallback(
        ...
        uint32_t r, uint32_t c, double contrast_parm, double sat_parm, double wexp_parm){
    //center rows
    for(int i = rSTART; i < rEND; i++){
        ...
        for(int j = cSTART; j < cEND; j++){
            ...
            _weights_inner(im,imC,at,real_at,top,left,right,bottom,Wn,sat_parm,contrast_parm,wexp_parm);
        }
    }
}
...
FORCE_INLINE void _weights_block(
    ...
    for(int i = rSTART+1; i < rEND-1; i++){
        ...
        for(int j = cSTART+1; j < cEND-1; j++){
            ...
            _weights_inner(tmp_weights,imC,at,real_at,top,left,right,bottom,Wn,sat_parm,contrast_parm,wexp_parm);
