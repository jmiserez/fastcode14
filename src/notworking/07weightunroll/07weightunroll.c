...
void weights(uint32_t nimages, uint32_t r, uint32_t c, double **I, double ***C, double **W, double *tmp_weights);
...
//    uint32_t nvals = mem->nvals;
    ...
    PERF_FUNC_ENTER
    weights(nimages,r,c,I,C,W,tmp_weights);
    PERF_FUNC_EXIT
...
void weights(uint32_t nimages, uint32_t r, uint32_t c, double **I, double ***C, double **W, double *tmp_weights){
    for (int n = 0; n < nimages; n++){
        ...
        //see fusion_includes.h
        int rNB = WEIGHTS_NB_R;
        int cNB = WEIGHTS_NB_C;
        if(rNB > r){
            rNB = r;
        }
        if(cNB > c){
        ...
        while(cSTART + cNB <= c){
            rSTART = 0;
            while(rSTART + rNB <= r){
                _weights_block_unroll2x(tmp_weights,
                               rSTART,
                               rSTART+rNB,
                               cSTART,
                               cSTART+cNB,
